...
private:
    ...
    std::atomic<bool> audio_stream_failed_;  // Flag to track if current stream failed (404, connection error)
    std::atomic<bool> skip_current_song_;    // Flag to skip current song and try next
